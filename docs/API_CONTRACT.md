...
**UI 使用頁面**：無 —— client `triggerBookAnalysis` 於 2026-09-05（PR #86）以零引用刪除。閱讀頁沒有這個按鈕；分析改由角色／事件的批次端點（#8b / #9e）觸發。
...
**UI 使用頁面**：無 —— client `regenerateAnalysis` 於 2026-09-05（PR #86）以零引用刪除。兩頁的「覆蓋重新生成」實際走各自的 `force` 參數（#8a / #9b）。
...
**UI 使用頁面**：無 —— 「共現詞」讀的是 #15i overview 的 `co_occurring_imagery`，不再逐一請求本端點（一頁一次 vs 每個象徵一次）。
...
**UI 使用頁面**：無 —— 前端沒有這個端點的 client（`api/symbols.ts` 有註明）。SEP 仍是活的，但只在**伺服器端**被 #15e 消費；本端點是供除錯／檢視用的 HTTP 外殼。
