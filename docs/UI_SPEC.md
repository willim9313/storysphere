...
- **目錄對照提示（TOC cross-check）**：純輔助、唯讀。閱讀欄中被判為 `toc` 的章節
  divider 下方出現置中提示框（`--accent` 邊框 + `--bg-secondary` 底）＋「解析目錄並對照」
  按鈕（✦ Sparkles，`--accent` 描邊輔助樣式）；**僅在有 `toc` 章節時出現**。點擊呼叫
  `#22d POST /books/:bookId/parse-toc`，由 AI 解析出書本自己聲明的章節清單與順序，從
