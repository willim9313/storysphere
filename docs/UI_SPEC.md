...
進入頁面
  → 載入事件清單
  → 依 ?event= 還原選取；無此參數則不預設選中任何事件

點擊事件
  → 寫入 ?event=（push，可用上一頁退回）→ 載入該事件分析（#7d）
...
見 [`docs/API_CONTRACT.md`](API_CONTRACT.md)：#6b（事件清單）、#6c（重新生成）、#7d（事件分析詳情）、#7e（觸發單一分析）、#7g（批次 EEP）、#8（任務 polling）
