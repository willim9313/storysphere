...
清單 item（卡片式，2026-07 更新）：
- 已分析：依名字首字 hash 出 entity 配色頭像 + 名稱（serif）+ 當前框架的原型名 +「提及 N 次」meta + 提及量迷你 bar（寬 `6+94·√(mentions/max)`%，accent）+ 綠色狀態點
- 未分析：muted 頭像 + 名稱（淡色）+「提及 N 次」meta + muted bar + 「建立」按鈕
