...
#### B-047 知識圖譜：非預設主題下節點類型識別困難
> 原 B-043，2026-06-30 重編：原號與已歸檔的「閱讀頁欄 2 章節搜尋」撞號。
...
**進度（2026-07-01，於 `chore/lint-cleanup` 分支）**:
- ✅ 後端 ruff：194 → **0**。config 遷移到 `[tool.ruff.lint]`；`--fix` 自動修 199 項；手動收尾 E741/B905/B904/B007/F841（B905 的 vectors 處用 strict=False 保原行為）
- ✅ 前端 eslint 安全子集：39 → **20**。config 加 `_`-前綴 ignore（清 19 個 unused-vars）；4 個 only-export-components 加帶理由 disable；移除 2 個冗餘 disable
- ✅ 附帶修復：3 個 tools 測試因 DI kwarg 漂移打真實 Gemini（429），改為正確注入 mock（非整合測試，不加 marker）
- ✅ 驗證：`pytest -m "not integration"` → 873 passed；`tsc` + `build` 通過

**剩餘（高風險，逐案評估）**: 前端 **20 個 react-hooks 類** eslint 錯誤 — `set-state-in-effect ×11`、`exhaustive-deps ×4`、`immutability ×2`、`preserve-manual-memoization ×2`、`refs ×1`。改錯會產生行為 bug，需逐一謹慎重構，不可批次 --fix。
...
| B-047 | 知識圖譜：非預設主題下節點類型識別困難 | 🟢 低 | 待開始（前置：F-17 已完成） |
| B-048 | Neo4j Link Prediction 支援缺口 | 🟢 低 | 待開始（前置：B-011） |
| B-049 | 累積 Lint 債清理（ruff + eslint） | 🟢 低 | 進行中（後端 0、前端剩 20 個 react-hooks 高風險） |
...
**維護者**: William
**最後更新**: 2026-06-30（解除 B-035/B-043/B-044 ID 撞號，開放項重編為 B-046/B-047/B-048，補列 B-042）
