...
- **判準已於 2026-09-11 在標註過的真實資料上試算**，見
  ...
  可低到 0.00、邊界可高到 0.33，任何門檻都切不開）；**`narrative_mode` 未轉換**是
  可用的第一層判準（ch7 兩個邊界全中、12 配對零偽陽性），但抓不到同一敘事層內的
  換場景
...
| B-068 | 事件抽取把同一場戲切成多個 event | 🟡 中 | 待開始（2026-09-10 已走查：不是重複而是節拍，處置改為場景分組；前置 B-106） |
