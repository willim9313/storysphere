...
**這比 plan 以為的更嚴重。** 那份文件的結論是「不能看穩定性，只能看效應大小」。但
單次呼叫就能差 50%，而該實驗要測的措辭效應是 +30% 到 +58%——**雜訊帶與效應帶重疊**。
所以那份實驗的效應大小在小 N 之下也不可靠，不只是穩定性指標失效。

**對之後的意思**：任何提示措辭的比較都必須先估供應端雜訊，再決定每組要跑幾次。
`20260910` 第六節「三個沒試過的方向」（兩段式抽取 / 顆粒度定義寫進 `event_type` /
`_parse_events` 後處理）在有雜訊基線之前做出來也分不出高下。

**本次測量自己的限制**: N=5、1 次離群，**不足以估出離群率**。要講「多少比例會變」需要
N≥30。本條只確立「會變」與「來源是供應端」。
...
| B-116 | `temperature=0` 之下 Gemini 仍然不可重現 | 🟡 中 | 待處理（2026-09-12 查明：來源是供應端，非 llm_retry。相同輸入 5 次得 2 種輸出、離群那次事件數多 50%，雜訊帶與措辭效應帶重疊。後續提示實驗需先估雜訊基線） |
