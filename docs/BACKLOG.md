...
**待辦內容**:
- 後端：`ruff check src/ --fix` 收自動修項，剩餘手動處理；順帶把 `pyproject.toml` 的 ruff 設定從過期鍵 `[tool.ruff] select` 遷移到 `[tool.ruff.lint] select`（目前會噴 deprecation 警告）
- 前端：`eslint src --fix` 收可修項，set-state-in-effect / react-refresh 類需逐一評估
- 自動修後須跑全測試（`pytest -m "not integration"` + `tsc` + `build`）確認無副作用
- 因橫掃檔案數多，建議獨立成 `chore(lint)` PR，不與功能變更混在一起
...
| B-049 | 累積 Lint 債清理（ruff + eslint） | 🟢 低 | 待開始 |
