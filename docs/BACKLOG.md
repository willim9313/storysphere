...
#### B-045 英雄旅程分析：前端缺乏觸發入口
**背景**: 後端 `POST /api/v1/narrative/hero-journey` endpoint 已完整實作（`src/api/routers/narrative.py`），schema 也已在 `generated.ts` 中，但前端沒有任何頁面或元件呼叫這個 endpoint。`frontend/src/api/` 也沒有 `narrative.ts` 封裝檔。建構概覽頁的 `hero_journey_stage` 節點因此永遠顯示「未建立」，即使章節摘要已完成也無法觸發。

**英雄旅程 vs 角色分析的差異**:
- 角色分析（CEP → archetype → arc）：逐個角色觸發，在角色分析頁有 UI
- 英雄旅程：整本書層面的 Campbell 12 階段敘事弧，依賴章節摘要（summaries），與角色分析完全獨立

**待辦內容**:
1. 新增 `frontend/src/api/narrative.ts`，封裝：
   - `triggerHeroJourney(bookId, language)` → `POST /books/{bookId}/narrative/hero-journey`
   - `fetchHeroJourney(bookId)` → `GET /books/{bookId}/narrative/structure`（取 hero_journey_stages 欄位）
2. 在建構概覽頁（`BuildOverviewPage.tsx`）的 `NodeDetail` 中，當 `nodeId === 'hero_journey_stage'` 且 `status !== 'complete'` 且無 blockers 時，將 disabled CTA 改為 active 觸發按鈕
3. 加上 task polling（複用 `useTaskPolling` hook）顯示分析進度
4. 可選：在事件分析頁（`EventAnalysisPage.tsx`）的敘事結構區塊旁加上英雄旅程觸發按鈕（與敘事結構同層，因為兩者都屬於 narrative analysis）

**前置依賴**: 章節摘要（summaries）已建立
...
**維護者**: William
**最後更新**: 2026-05-30（新增 B-045 英雄旅程前端觸發入口）
