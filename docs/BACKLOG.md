...
**待辦內容**:
- 確認事件抽取是否對同一 chunk / 同一場景重複產出 event（需連 Neo4j 查，本次未做）
- 決定去重時機：抽取當下合併，還是抽取後加一道 dedup
- `Event` 目前沒有 chunk / scene 錨點（只有 `chapter` 與 `narrative_position`），若要可靠去重可能需補

**觸發時機**: 下次動到 ingestion / event extraction 時，或證據重複造成誤判回報時。

---

#### B-069 張力證據「同場景摺疊」無可用判準
**背景**: 設計稿（`docs/handoff/20260731-tension-page`）要求證據區顯示「6 → 3 則」與逐字對照，需要後端提供 `scene_group_id`。2026-08-02 議定判準為「同章 + 引文集合 Jaccard ≥ 0.5」，**在真實資料上驗證失敗**：`名字的潮汐` 38 個 TEU 在門檻 0.3 / 0.4 / 0.5 / 0.6 / 0.7 全部產出 0 組。
...
**前置**: 需先確認後端 `#14a` / `#14b` 的 task 結果是否留有失敗 event 的清單與原因；若沒有，這是後端先要補的部分。

**觸發時機**: 組裝失敗率造成實際困擾時，或下次動到 TEU 組裝管線時。
...
**維護者**: William
**最後更新**: 2026-07-16（新增 B-059：派系語意命名，源自角色頁翻新的派系分群討論）
