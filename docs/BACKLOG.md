...
| B-068 | 事件抽取把同一場戲切成多個 event | 🟡 中 | 🔶 部分完成（2026-09-11 PR #107 第一層落地，但端到端實測推翻了判準命名——`narrative_mode` 未轉換預設合併而非保守，已改名 `narrative_run_index`；真正的場景邊界要段落錨點，第二層待排，見條目） |
