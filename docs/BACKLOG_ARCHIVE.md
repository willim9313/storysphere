...
- `src/storysphere/services/epistemic_state_service.py`：計算角色認識論狀態
...
**已實作內容**:
- `src/storysphere/domain/voice.py`：`VoiceFingerprint` Pydantic model（定量指標 + LLM 質性描述 + 代表性引文）
- `src/storysphere/services/voice_profiling_service.py`：用 Qdrant 語意搜索 + 量化特徵提取 + LLM 質性描述
...
**實作**: `src/storysphere/domain/symbol_analysis.py`（SymbolInterpretation）, `src/storysphere/services/symbol_analysis_service.py`, `src/storysphere/agents/analysis_agent.py`（analyze_symbol）, `src/storysphere/api/routers/symbols.py`（analyze/interpretation endpoints）, `src/storysphere/api/routers/unraveling.py`（symbol_analysis_result node）
...
**實作**: `src/storysphere/services/kg_service_base.py`, `kg_service_neo4j.py`, `kg_migration.py`; `src/storysphere/api/routers/kg_settings.py`; `frontend/src/pages/SettingsPage.tsx`
...
**實作**: `src/storysphere/api/routers/relations.py`，已掛載至 `main.py`
...
**實作**:
- `src/storysphere/core/tracing.py` — `configure_langfuse()` + `get_langfuse_handler()` singleton
- `src/storysphere/agents/chat_agent.py` — `ainvoke`/`astream` 注入 `CallbackHandler`
- `src/storysphere/agents/analysis_agent.py` — `@_langfuse_observe` 取代 `@traceable`
...
**實作**: `src/storysphere/api/routers/metrics.py`，直接呼叫 `get_metrics().get_stats()`
...
**相關檔案**: `src/storysphere/services/keyword_service.py` — `LLMKeywordExtractor._parse_response()`
...
**實作**:
- 更新 `src/storysphere/domain/models.py` EventNode schema，新增三個欄位：`tension_signal`, `emotional_intensity`, `emotional_valence`
- 更新 `src/storysphere/pipelines/entity_extractor.py` 的 Event 提取 prompt
...
**前置依賴**: B-024
**實作**: `src/storysphere/pipelines/concept_inference.py` — 輸入候選段落群，LLM 產出帶置信度的 Concept 標籤，存入 KG
...
**實作**:
- `src/storysphere/domain/tension.py`：`TensionPole`, `TEU`, `TensionLine`, `TensionTheme` Pydantic models
- `src/storysphere/services/tension_service.py`：`assemble_teu(event_id)` + 存取層
...
- `src/storysphere/config/mythos.py`（Frye/Booker 標籤，類比 `archetypes.py`）
...
- 更新 `src/storysphere/domain/models.py` EventNode，新增：`narrative_weight`, `narrative_weight_source`, `story_time`
...
**實作**: `src/storysphere/services/extraction_service.py` — `story_time_hint` 欄位已在 Event 提取 prompt 中（確認實作時發現已完成）
...
**實作**:
- `src/storysphere/domain/narrative.py`：`NarrativeStructure`, `HeroJourneyStage`, `ProppFunctionRef`, `KernelSatelliteResult`
- `src/storysphere/services/narrative_service.py`：`classify_by_heuristic(document_id)`, `get_kernel_spine(document_id)`
...
**實作**:
- `src/storysphere/config/hero_journey.py`：12 階段 loader + `get_hero_journey_summary()`
- `src/storysphere/config/hero_journey/hero_journey_{en,zh}.json`：階段定義
...
**實作**:
- `src/storysphere/api/schemas/narrative.py`：request schemas
- `src/storysphere/api/routers/narrative.py`：9 個端點（async classify/refine/hero-journey + polling + sync kernel-spine + GET/PATCH structure）
...
**實作**:
- `src/storysphere/domain/narrative.py`：`TemporalAnalysis`, `TemporalDisplacement`
...
**實作**:
- `src/storysphere/domain/symbol_analysis.py`：`SEP`（Symbol Evidence Profile）+ `SEPOccurrenceContext`，欄位包含 `imagery_id`, `book_id`, `term`, `imagery_type`, `frequency`, `occurrence_contexts`（段落文字 + 章節位置）、`co_occurring_entity_ids`、`co_occurring_event_ids`、`chapter_distribution`、`peak_chapters`
...
**實作**: `src/storysphere/domain/symbol_analysis.py`, `src/storysphere/services/symbol_service.py`, `src/storysphere/api/routers/symbols.py`, `src/storysphere/api/routers/unraveling.py`
...
**實作**: `src/storysphere/api/routers/unraveling.py`; `frontend/src/api/unraveling.ts`
...
**實作**:
- `src/storysphere/domain/imagery.py`：`ImageryType` enum、`ImageryEntity`、`SymbolOccurrence`、`SymbolCluster`（純 Pydantic）
- 持久層：`src/storysphere/services/symbol_service.py`（aiosqlite，兩張表：`imagery_entities` + `symbol_occurrences`）
...
**實作**:
- `src/storysphere/services/imagery_extractor.py`：LLM 提取 + 貪心余弦相似度聚類（EmbeddingGenerator）
- `src/storysphere/pipelines/symbol_discovery/pipeline.py`：`SymbolDiscoveryPipeline(BasePipeline)`，章節順序處理
- `src/storysphere/workflows/ingestion.py`：新增 Step 3b（progress=75），`skip_symbols=True` 可跳過；`IngestionResult.imagery_extracted`
...
**實作**:
- `src/storysphere/services/symbol_graph_service.py`：`SymbolGraphService`，on-demand `build_graph()`，NetworkX `DiGraph`
...
**實作**:
- `src/storysphere/api/schemas/symbols.py`：`ImageryEntityResponse`、`ImageryListResponse`、`SymbolTimelineEntry`、`CoOccurrenceEntry`（snake_case）
- `src/storysphere/api/routers/symbols.py`：`GET /symbols`、`GET /symbols/{id}/timeline`、`GET /symbols/{id}/co-occurrences`
- `src/storysphere/api/deps.py`：`SymbolServiceDep`、`SymbolGraphServiceDep`
...
**已實作**:
- `src/storysphere/config/settings.py`：新增 `deploy_mode: Literal["lightweight", "standard"] = "lightweight"`、`qdrant_local_path`；lightweight 模式強制 `kg_mode=networkx` 並 log warning
- `src/storysphere/services/vector_service.py`：依 `deploy_mode` 決定 Qdrant client（local file path vs. remote URL）；standard 模式連線失敗拋明確錯誤
- `src/storysphere/api/main.py`：lifespan 啟動時針對 lightweight 模式發出多 worker 警告
...
**已實作**:
- `src/storysphere/config/settings.py`：新增 `primary_llm_provider: Literal["gemini", "openai", "anthropic", "local"] = "gemini"`
- `src/storysphere/core/llm_client.py`：`_resolve_primary()` 改為直接讀取 `settings.primary_llm_provider`；指定 provider 的 key 未設定時啟動報明確錯誤，不靜默降級
