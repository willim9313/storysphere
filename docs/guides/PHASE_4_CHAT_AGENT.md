...
> 2. **移除了 `QueryPatternRecognizer` 的「快速路由」**——recognizer 不再於高信心時直接呼叫 composite tool 並返回；現在**只做實體追蹤**（更新 `current_focus_entity` 供代名詞解析），回應一律由 agent loop 生成，`suggested_tools` 已無消費者。
>
> 下方相關段落標注 🔸 表示與現況不符。
...
1. 🔸 ~~**使用 `create_react_agent` 而非自訂 `StateGraph`**~~ — **實作已改為手動 `StateGraph`**（`agent` 節點 + `ToolNode`，`tools_condition` 條件路由），以保留對訊息串流的控制。
...
backend/storysphere/tools/composite_tools/
...
2. **新增使用者訊息** 至 state
3. 🔸 **實體追蹤**（原為「快速路由」，已移除）— `QueryPatternRecognizer.recognize()`，若信心值 > 0.8 只更新實體追蹤狀態（`current_focus_entity`，供代名詞解析）；**不再直接呼叫工具返回**
4. **Agent loop** — `self._graph.ainvoke({"messages": [...]})`（回應一律由此生成）
