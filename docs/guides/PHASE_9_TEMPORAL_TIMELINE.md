...
**檔案**：`backend/storysphere/domain/events.py`
...
**檔案**：`backend/storysphere/domain/temporal.py`（新建）
...
**檔案**：`backend/storysphere/workflows/ingestion.py`
...
| 檔案 | 說明 |
|------|------|
| `backend/storysphere/domain/temporal.py` | `TemporalRelation`, `TemporalRelationType` |
| `backend/storysphere/services/global_timeline_service.py` | DAG 建構、cycle 解決、rank 計算 |
| `backend/storysphere/agents/timeline_agent.py` | Phase 2 LLM 編排，含分批處理 |
| `backend/storysphere/pipelines/temporal_pipeline.py` | Ingestion 步驟 6 的 pipeline 包裝 |
| `backend/storysphere/tools/graph_tools/get_global_timeline.py` | 全域時間線查詢 Tool |
...
| 檔案 | 修改內容 |
|------|---------|
| `backend/storysphere/domain/events.py` | 加入 `NarrativeMode`、4 個新欄位 |
| `backend/storysphere/services/extraction_service.py` | Phase 1 prompt 擴充，解析新欄位 |
| `backend/storysphere/services/kg_service.py` | 支援按 `chronological_rank` 排序；儲存 TemporalRelation |
| `backend/storysphere/workflows/ingestion.py` | 加入步驟 6（TemporalPipeline） |
| `backend/storysphere/tools/graph_tools/get_entity_timeline.py` | 新增 `sort_by` 參數（`narrative` / `chronological`） |
| `backend/storysphere/api/routers/books.py` | 新增 `GET /books/{id}/timeline` |
...
- 新建 `backend/storysphere/domain/temporal.py`
- 擴充 `backend/storysphere/domain/events.py`
