...
`src/storysphere/api/schemas/common.py` 的 `TaskStatus` 新增 3 個**選填**欄位：
...
`src/storysphere/api/routers/tasks.py`：
...
- 修改 `src/storysphere/api/schemas/common.py`（TaskStatus 三欄位 + create 簽章影響）
- 修改 `src/storysphere/api/store.py`（兩 backend 的 `create` 參數、`list()`、created_at、SQLite 欄位遷移）
- 新增 store 測試

**子任務 B — 後端端點 + 契約**
- 修改 `src/storysphere/api/routers/tasks.py`（`GET /tasks`）
