...
### `HeroJourneyStage`（`backend/storysphere/domain/narrative.py`）
...
### `NarrativeStructure`（`backend/storysphere/domain/narrative.py`）
...
3. **階段文案來源** — `backend/storysphere/config/hero_journey/hero_journey_zh.json` 和 `hero_journey_en.json`，不要在前端硬寫；`frontend/src/data/frameworksData.ts` 的 `hero_journey` key 有部分重疊內容可參考
...
- `backend/storysphere/config/hero_journey/hero_journey_zh.json` — 12 個階段中文定義與描述
- `backend/storysphere/config/hero_journey/hero_journey_en.json` — 英文版
- `backend/storysphere/domain/narrative.py` — 完整 domain model
- `docs/API_CONTRACT.md` — #21e、#21f、#21k
- `backend/storysphere/api/routers/narrative.py` — endpoint 實作細節
