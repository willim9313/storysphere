...
### `HeroJourneyStage`（`src/domain/narrative.py`）
...
### `NarrativeStructure`（`src/domain/narrative.py`）
...
3. **階段文案來源** — `src/config/hero_journey/hero_journey_zh.json` 和 `hero_journey_en.json`，不要在前端硬寫；`frontend/src/data/frameworksData.ts` 的 `hero_journey` key 有部分重疊內容可參考
...
- `src/config/hero_journey/hero_journey_zh.json` — 12 個階段中文定義與描述
- `src/config/hero_journey/hero_journey_en.json` — 英文版
- `src/domain/narrative.py` — 完整 domain model
- `docs/API_CONTRACT.md` — #21e、#21f、#21k
- `src/api/routers/narrative.py` — endpoint 實作細節
