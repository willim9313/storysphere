...
| 檔案 | 說明 |
|------|------|
| `backend/storysphere/domain/faction.py` | `Faction`、`FactionRelation`、`FactionAnalysis` Pydantic models（snake_case） |
| `backend/storysphere/services/faction_service.py` | `FactionService.detect_factions(book_id, chapter?)` — 純圖計算，無 LLM |
| `backend/storysphere/api/schemas/factions.py` | `FactionAnalysisResponse`（camelCase via `alias_generator=to_camel`） |
| `backend/storysphere/api/routers/factions.py` | `GET /books/{book_id}/analysis/factions?chapter=<int>` |

### 後端（修改檔案）

| 檔案 | 修改項 |
|------|--------|
| `backend/storysphere/api/deps.py` | 新增 `get_faction_service()` (`@lru_cache(maxsize=1)`)、`FactionServiceDep` |
| `backend/storysphere/api/main.py` | import `factions` router + `app.include_router(factions.router, prefix=prefix)` |
...
### `backend/storysphere/domain/faction.py`
```python
...
### `backend/storysphere/services/faction_service.py` 核心邏輯
...
### `backend/storysphere/api/routers/factions.py`
```
...
Phase 1（後端）：
  backend/storysphere/domain/faction.py
  backend/storysphere/services/faction_service.py
  backend/storysphere/api/schemas/factions.py
  backend/storysphere/api/routers/factions.py
  backend/storysphere/api/deps.py + backend/storysphere/api/main.py
  npm run gen:types
  tests/
