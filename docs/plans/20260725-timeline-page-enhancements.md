...
**狀態**：✅ **已落地**（2026-07-27，branch `feat/timeline-page-revamp`）。
設計稿已回，全頁重做完成；落地版規格見 [`docs/UI_SPEC.md` §3.7](../UI_SPEC.md)。

> **落地與本文件的差異（以 UI_SPEC §3.7 為準）**
>
> | 本文件原訂 | 實際落地 | 原因 |
> |---|---|---|
> | Phase 1 P1-3 「時序關係」改名為「上下文位置」 | **整區移除** | 設計稿的面板沒有這一區；在主打時序的頁面上，改名仍不如拿掉 |
> | Phase 2 P2-2 EEP 引言跳原文 | **未做** | EEP accordion 在新面板中不存在；跳原文改掛在事件概要（P2-1 已做） |
> | Phase 3 P3-3 RWD 1440/1280/1024 | **未做** | 設計稿是固定 1440、窄於此橫向捲動，未涵蓋另兩個寬度（見 §7 未決 5） |
> | Phase 4 泳道「第四張卡 vs 疊加」 | **疊加模式** | 設計稿的建議；泳道是同一條 X 軸的第二種讀法，不是時間軸的替代品 |
> | §2.1 layout 水平／垂直切換 | **移除** | 新骨架是固定寬多行譜，「排列方向」不再指涉任何東西 |
>
> **設計稿有一處與實測資料矛盾，未照做**：決策稿把「倒敘與預敘」的解鎖條件設為
> 「故事時序跑完 62/62」，但 §1.2／§2.3 已實測 `storyTimeHint` 覆蓋率 14.5%
> **且跑時序計算不會改善**（資料來源不同）。落地版改用真實的 `coverage_sufficient`
> 作為 gate，並在阻擋文案中明講「跑故事時序不會增加」。
...
| ~~4~~ | ~~§1.2 的 rank 填充率實測結果若偏低，Phase 4 是否延後？~~ | ✅ 未延後：泳道以 `index` 定位，不依賴 rank |
| **5** | **RWD 1440 / 1280 / 1024 另開任務**——設計稿只給 1440，另兩個寬度沒有設計背書 | 使用者已決（本輪不做） |
| **6** | `timeline.*` 有 88 個 i18n key 因本次移除 UI 而成孤兒（Genette banner／chip、EEP／因果／影響 accordion、prior/subsequent、layout 切換、舊矩陣標籤）。**未刪**，待確認後清理 | 使用者 |
| **7** | `d3` 現在無人使用（唯一消費者 `MatrixCanvas` 已改寫）。**未從 package.json 移除**（超出本任務範圍） | 使用者 |
