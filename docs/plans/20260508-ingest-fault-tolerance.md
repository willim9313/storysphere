...
| 檔案 | 變動 |
|------|------|
| `backend/storysphere/domain/documents.py` | 新增 `StepStatus`、`PipelineStatus`、`Document.pipeline_status` |
| `backend/storysphere/services/document_service.py` | DB migration、`save_document` / `get_document` 讀寫 `pipeline_status_json`、新增 `update_pipeline_status()` |
| `backend/storysphere/workflows/ingestion.py` | 提前 `save_document()`，各步驟後更新 status，KG save 條件調整 |
| `backend/storysphere/api/task_registry.py` | 新增（in-process asyncio.Task registry） |
| `backend/storysphere/api/routers/books.py` | 改用 `asyncio.create_task()`，新增 rerun endpoints |
| `backend/storysphere/api/routers/tasks.py` | 新增 `POST /tasks/:taskId/cancel` |
| `backend/storysphere/api/schemas/books.py` | `BookResponse` 新增 `pipelineStatus` |
