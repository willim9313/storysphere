...
| 檔案 | 類型 | 改動摘要 |
|------|------|---------|
| `src/storysphere/domain/documents.py` | 修改 | `Paragraph` 新增 `title_span: tuple[int, int] \| None = None` |
| `src/storysphere/api/schemas/common.py` | 修改 | `TaskStatus.status` Literal 加入 `"awaiting_review"` |
| `src/storysphere/api/store.py` | 修改 | `MemoryTaskStore` / `SQLiteTaskStore` 新增 `set_awaiting_review()`；cleanup SQL 排除此狀態 |
| `src/storysphere/api/review_registry.py` | **新增** | `asyncio.Event` registry，管理 pause/resume + reviewed chapters 傳遞 |
| `src/storysphere/pipelines/document_processing/pipeline.py` | 修改 | Step 3 前拼接 title；chunker 後設 `title_span` |
| `src/storysphere/services/document_service.py` | 修改 | `_ParagraphRow` 新增 `title_span_json` 欄位（migration）；新增 `get_review_data()`；新增 `replace_chapters()` |
| `src/storysphere/workflows/ingestion.py` | 修改 | Step 1b 存檔後插入 `await review_registry.wait(book_id)`；收到 reviewed chapters 後重組 `doc.chapters`；繼續步驟 2+ |
| `src/storysphere/api/schemas/books.py` | 修改 | 新增 `ReviewDataResponse`、`ReviewParagraphResponse`、`ReviewChapterInput`、`ReviewSubmitRequest` |
| `src/storysphere/api/routers/books.py` | 修改 | 新增 `GET /{book_id}/review-data`（僅 `awaiting_review` 狀態可存取）；新增 `POST /{book_id}/review` |
...
```python
# src/storysphere/api/review_registry.py
import asyncio
