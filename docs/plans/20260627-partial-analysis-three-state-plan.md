...
**Files:**
- Create: `src/storysphere/core/gather_parts.py`
- Create: `tests/core/test_gather_parts.py`
- Modify: `src/storysphere/services/analysis_models.py` (add `failed_parts` to `CharacterAnalysisResult` ~line 68, `EventAnalysisResult` ~line 152)
...
```python
# src/storysphere/core/gather_parts.py
...
In `src/storysphere/services/analysis_models.py`, add to `CharacterAnalysisResult` (after `coverage`, before `analyzed_at`):
...
```bash
git add src/storysphere/core/gather_parts.py tests/core/test_gather_parts.py src/storysphere/services/analysis_models.py
...
**Files:**
- Modify: `src/storysphere/services/analysis_service.py` (`analyze_character` ~267-363; add `_character_parts` helper)
- Modify: `src/storysphere/agents/analysis_agent.py` (`analyze_character` ~56-117)
...
In `src/storysphere/services/analysis_service.py`, add a helper and rewrite the body of `analyze_character`. Replace the block from `# Step 1: Extract CEP` (~303) through the `return CharacterAnalysisResult(...)` (~363) with:
...
In `src/storysphere/agents/analysis_agent.py` `analyze_character` (~56), add `retry_parts: list[str] | None = None` to the signature, and before the existing cache-first block insert a partial-mode branch:
...
```bash
git add src/storysphere/services/analysis_service.py src/storysphere/agents/analysis_agent.py tests/services/test_analysis_partial.py
...
**Files:**
- Modify: `src/storysphere/services/analysis_service.py` (`analyze_event` ~617-689; add `_event_parts`)
- Modify: `src/storysphere/agents/analysis_agent.py` (`analyze_event` ~125-190)
...
In `src/storysphere/services/analysis_service.py`, replace the parallel block (~643-688) with named parts. Add params to the signature (mirroring Task B). Replace from `# causality and impact are independent` through the `return EventAnalysisResult(...)`:
...
In `src/storysphere/agents/analysis_agent.py` `analyze_event` (~125), add `retry_parts: list[str] | None = None`, and after `cache_key = f"event:..."` insert:
...
**Files:**
- Modify: `src/storysphere/api/schemas/books.py` (`CharacterAnalysisDetailResponse`: add `status`, `failed_parts`; trigger request: add `mode`)
- Modify: `src/storysphere/api/routers/books.py` (`get_entity_analysis` ~1449; `trigger_entity_analysis` ~1515 / `_run_entity_analysis` ~209; event equivalents `get_event_analysis` ~1858 / `trigger_event_analysis` ~1824)
...
(Confirm the dependency name `deps.get_analysis_cache` matches `AnalysisCacheDep` in `src/storysphere/api/deps.py`; adjust if different.)
...
In `src/storysphere/api/schemas/books.py`, add to `CharacterAnalysisDetailResponse`:
...
In `src/storysphere/api/routers/books.py` `get_entity_analysis` (~1467), add to the `CharacterAnalysisDetailResponse(...)` construction:
...
Define a request body schema in `src/storysphere/api/schemas/books.py`:
...
ruff check src/storysphere/api/routers/books.py src/storysphere/api/schemas/books.py
git add src/storysphere/api/routers/books.py src/storysphere/api/schemas/books.py docs/API_CONTRACT.md frontend/src/api/generated.ts tests/api/test_analysis_partial_api.py
...
npx eslint src/components/analysis/sections/PersonaPane.tsx src/pages/CharacterAnalysisPage.tsx src/storysphere/api/analysis.ts src/components/tasks/TaskRow.tsx
cd ..
...
- `EventSummary` / `ImpactAnalysis` exact field names: confirm against `src/storysphere/services/analysis_models.py` before writing the event empty-defaults (Task C Step 3) — copy the field set already used in the current `analyze_event` empty branches.
