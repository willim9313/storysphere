...
### 3. 修改的檔案
**後端**
- `src/storysphere/api/schemas/common.py` — 新增 `MurmurEvent` model、`TaskStatus.murmur_events`
- `src/storysphere/api/store.py` — 兩個 backend 新增 `task_murmur_events` 儲存與 delta query；新增 async `append_murmur`；`set_completed` 前 drain
- `src/storysphere/api/routers/tasks.py` — `?after=N` query param
- `src/storysphere/workflows/ingestion.py` — 新增 `murmur_cb` 參數，串接各 pipeline
- `src/storysphere/api/routers/books.py` — `_run_ingestion` 傳入 `murmur_cb`（至少 2 處）
- `src/storysphere/pipelines/document_processing/pipeline.py` — emit pdfParsing 事件
- `src/storysphere/pipelines/summarization/pipeline.py` — emit summarization 事件
- `src/storysphere/pipelines/knowledge_graph/pipeline.py` — emit featureExtraction entity 事件
- `src/storysphere/pipelines/symbol_discovery/pipeline.py` — emit symbolExploration 事件
...
```python
# src/storysphere/api/schemas/common.py
...
- `src/storysphere/api/routers/books.py` 的 `_run_ingestion`（至少 2 處）
