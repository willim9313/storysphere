...
**單筆觸發** `src/api/routers/books.py:1510-1534`：
```
...
### 後端
- `src/api/routers/books.py` — 新增 `_run_batch_entity_analysis` 與 `trigger_batch_entity_analysis`
...
**不會動的檔案**：
- `src/services/analysis_service.py`（`analyze_character` 簽章不變）
- `src/services/analysis_models.py`
- React Query key 結構
