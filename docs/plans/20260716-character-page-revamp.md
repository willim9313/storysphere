...
| Batch | 內容 | 前置 |
|-------|------|------|
| 1(P0 小修 + 資料修復,可先行,不等設計稿) | **#0 提及數修復(後端)**、#6 清單排序/搜尋/提及量 bar、#9 誤信欄過濾、#8 Voice 狀態(含小後端)、#12 生成中 checklist、#13 鍵盤操作 | 無(#6 的 bar 與排序依賴 #0) |
| 2(設計稿後) | #1 角色總覽 landing(排行/象限雙視圖)、#7 Arc 時間軸、#10 認知對照視圖、#11 分層批次、#14 原型篩選 | #0 + centrality 端點 + Claude Design canvas(已交付) |
| 3(跨頁導航) | #3 關係可點(含 target 分組)、#5 keyEvents 互通(v1 名稱比對) | 設計稿(affordance) |
| 4(溯源跳轉) | #2 引言溯源、#4 認知事件跳轉(含 marker 聚合) | **reader-revamp 合併** + 設計稿 |
