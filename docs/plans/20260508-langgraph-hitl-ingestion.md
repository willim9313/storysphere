...
### 新增
- `backend/storysphere/workflows/ingestion_graph.py` — IngestionState + 3 nodes + `build_ingestion_graph()`
- `docs/plans/20260508-langgraph-hitl-ingestion.md` — 本文件

### 修改
- `backend/storysphere/workflows/ingestion.py` — 拆出 `run_phase1()` / `run_phase2(doc_id)`；`run()` 改為直接呼叫 phase1+phase2（無 review 暫停）
- `backend/storysphere/api/store.py` — 兩個 store 各加 `get_task_id_by_book_id(book_id)`；module-level `get_task_id_by_book_id()` async helper
- `backend/storysphere/api/deps.py` — 加 `get_ingestion_graph()` / `set_ingestion_graph()` singleton
- `backend/storysphere/api/main.py` — lifespan 初始化 `AsyncSqliteSaver` + `build_ingestion_graph()`
- `backend/storysphere/api/routers/books.py` — `POST /upload` 改用 graph；review endpoints 改查 task_store；`POST /review` 改 Command(resume) + optimistic set_running
- `backend/storysphere/config/settings.py` — `task_store_backend` 預設改 `"sqlite"`；加 `ingestion_checkpoint_db_path`
- `tests/api/test_chapter_review.py` — 移除 TestReviewRegistry；更新 endpoint tests 使用 task_store

### 刪除
- `backend/storysphere/api/review_registry.py` — 完全被 LangGraph interrupt 取代
