...
**判準已定（2026-08-02）：同章節 + 引文集合 Jaccard 重疊率 ≥ 0.5 視為同場景。** 規格如下：
...
**驗證要求：** Phase 1b 動工時先在 `名字的潮汐` 的 38 個 TEU 上跑一次分群並人工看結果（零 LLM 成本，直接讀 `var/analysis_cache.db`）。若 0.5 的分群明顯不合理，**調門檻不改演算法**，並把實測值回填本表。
...
| `scene_group_id` | 2.2 | 判準已定：**同章 + 引文集合 Jaccard ≥ 0.5**，完整規格見 §2.2 的判準表 |
...
| **Phase 1b** | **第二批後端契約**（§5.3 四項全做）：`flipped`、`TensionLine` provenance、指派 endpoint、`edit` 紀錄、`reviewed_count_at_synth`、`scene_group_id`（判準見 §2.2） | **新增，擋住 Phase 3。四項待決全部已定，無阻塞** |
