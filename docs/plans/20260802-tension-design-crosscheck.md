...
**決定（2026-08-04）：保留。** 核准 / 改命題 / 拒絕三顆按鈕留在新 hero 的 footer，`#14j` 維持有呼叫端。理由是設計稿未必是刻意拿掉，而砍掉現有可用功能應該是獨立決定。程式碼中已標註這一段不在 canvas 內。

與 §5.1 的 Frye / Booker 同屬「設計稿沒畫、我們自行保留」，Ink 主題下需一併驗收。
...
| Phase 3 | 依 `.dc.html` canvas 實作前端 | ✅ 完成（2026-08-05，6 個子項全數落地） |
| Phase 4 | P4-2 RWD、P1-9 a11y、P0-5 失敗清單 | ⏭ 移出本輪，轉 BACKLOG（B-070 / B-071 / B-072） |

Phase 3 內部沿用設計稿建議的順序（審核主動線價值最高，安全網不要放最後），6 個子項各自獨立 commit：

| # | 子項 | commit |
|---|------|--------|
| 1 | 張力線表格 + 篩選/排序 + 審核抽屜 | `27dae6d` / `25c6c80`（+ `148e993` 修深連結與遮蔽） |
| 2 | Pipeline stepper + 各狀態卡（empty / step1 / running / error） | `5a5eab0` / `7a53d61` |
| 3 | 章節格點 + 未歸入列 | `685fde3` |
| 4 | TEU 逐章模式 | `bc37a55` |
| 5 | 鍵盤快捷鍵 + 批次操作 | 併入 `27dae6d` / `25c6c80` |
| 6 | 重跑確認框 + stale 狀態 | `b7a779d` / `385bdf4` |

**Phase 4 為何不併進本輪**：三項都不是「翻新沒做完」，而是各自需要獨立決策——
RWD 要先決定抽屜在窄視窗的行為與格點的收斂策略（設計交付包未涵蓋 1280 / 1024）；
a11y 要處理格點與迷你柱狀圖的非視覺替代；P0-5 需要後端先能回報哪些 TEU 組裝失敗。
硬塞進本輪會讓已可驗收的主動線延後，故轉為獨立 backlog 條目。

> 交付現況與已知缺口另記於 `docs/UI_SPEC.md` §3.8「已知缺口」。
