...
| `src/storysphere/api/schemas/kg_settings.py` | 修改 | `KgStatusResponse` 加入 `deploy_mode` 欄位 |
| `src/storysphere/api/routers/settings_info.py` | 新建 | `GET /api/v1/settings/info` 唯讀 endpoint |
| `src/storysphere/api/schemas/settings_info.py` | 新建 | `SettingsInfoResponse` schema |
