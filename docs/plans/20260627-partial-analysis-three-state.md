...
新增小工具（放 `src/storysphere/core/`，例如 `gather_parts.py`）：
...
**子任務 A — 共用機制**
- 新增 `src/storysphere/core/gather_parts.py` + 測試
- 修改 `src/storysphere/services/analysis_models.py`（`failed_parts` 欄位 / 共用 base）

**子任務 B — character pipeline**
- 修改 `src/storysphere/services/analysis_service.py`（`analyze_character` 用 `gather_parts`、加 `retry_parts`）
- 修改 `src/storysphere/agents/analysis_agent.py`（透傳 `retry_parts` + 部分重算路徑）
- service 測試
...
**子任務 D — API + 契約**
- 修改 `src/storysphere/api/routers/books.py`（character/event 觸發端點加模式、GET 回應加 `status`/`failedParts`；event 端點同步）
- 修改對應 `src/storysphere/api/schemas/`（response 加欄位、request 加 mode）
