...
| [`src/storysphere/api/routers/symbols.py`](../../src/storysphere/api/routers/symbols.py) | 後端 router（8 endpoints，全部已有 `response_model=`） | 不需動 |
| [`src/storysphere/api/schemas/symbols.py`](../../src/storysphere/api/schemas/symbols.py) | `ImageryEntityResponse` / `ImageryListResponse` / `SymbolTimelineEntry` / `CoOccurrenceEntry` | 不需動 |
| [`src/storysphere/domain/imagery.py`](../../src/storysphere/domain/imagery.py) / [`src/storysphere/domain/symbol_analysis.py`](../../src/storysphere/domain/symbol_analysis.py) | `ImageryEntity` / `SymbolOccurrence` / `SEP` / `SymbolInterpretation` 等 domain model | 不需動 |
...
4. **`co_occurrences` 自動 build 行為**：[`src/storysphere/api/routers/symbols.py`](../../src/storysphere/api/routers/symbols.py) L108–L110 顯示第一次請求時會 auto-build graph，可能耗時數秒。設計時 loading 視覺要能撐住這段時間（目前用 `LoadingSpinner` 是夠的，但若改 dashboard 模式預載入需考慮）。
