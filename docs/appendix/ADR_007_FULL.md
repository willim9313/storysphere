...
舊版 `extract_json_from_text()`（`old_version/src/storysphere/core/utils/output_extractor.py`）實現了 4 步 fallback chain：
...
- [ ] 將 `extract_json_from_text()` 移植到 `src/storysphere/core/utils/output_extractor.py`
...
舊版 `DataSanitizer`（`old_version/src/storysphere/core/utils/data_sanitizer.py`）提供了：
...
- [ ] 將 `DataSanitizer` 移植到 `src/storysphere/core/utils/data_sanitizer.py`
