...
- **禁止在專案目錄內執行會刪檔的批次指令** — 判準與作法見下一節

---

## 破壞性指令

`rm -rf`、`rsync --delete`、`git clean -fdx`、`git checkout -- .`、`git reset --hard`
這一類**會刪掉或覆蓋「你沒有明確列出」的檔案**的指令：

- **備份放到專案目錄外**（用 scratchpad）。備份與正本在同一個會被刪除的範圍內，
  等於沒有備份。
- **來源與目的地的排除清單必須逐項核對。** `--delete` 的語意是「來源沒有、目的地
  有就刪掉」，兩邊 exclude 不一致，差集就是被刪的東西。
- **先看會動到什麼**：`rsync --dry-run`、`git clean -n`。沒有 dry-run 的指令就先
  `ls` 一次目標範圍。
- **`var/` 是使用者資料，預設當成不可逆。** 它被 `.gitignore` 忽略，git 救不了它
  ——與 `npm ci` 就能回來的 `node_modules` 性質不同。
- **還原工作樹用 `git stash` 或另開 checkout**，不要用 rsync 對著專案目錄掃。
...
CI 存在不代表可以不看。B-066 是反例：`tsc -b` 長期紅著，紅久了就沒人看，錯誤一路累積到
10 個才被清掉——其中還混著一個已經不存在的 interface 引用。同樣地，2026-08-21 才發現
`ruff check tests/` 紅著 126 條而且**從來沒綠過**。閘門會腐化，而腐化是靜默的；紅燈亮著
沒人管，和沒有紅燈是一樣的。
