...
class MultilingualTemplateManager:
    ...
    def _load_templates(self):
        ...
        self._templates[Language.CHINESE] = {
            TaskType.SUMMARIZATION: ChineseTaskTemplates.get_summarization_template(),
            TaskType.KEYWORD_EXTRACTION: ChineseTaskTemplates.get_keyword_extraction_template(),
        }
        
        # 載入英文模板
        self._templates[Language.ENGLISH] = {
            TaskType.SUMMARIZATION: EnglishTaskTemplates.get_summarization_template(),
            TaskType.KEYWORD_EXTRACTION: EnglishTaskTemplates.get_keyword_extraction_template(),
        }
    
    def get_template(self, task_type: TaskType, language: Language = None) -> BaseTemplate:
        """獲取指定任務和語言的模板"""
        lang = language or self.default_language
        return self._templates.get(lang, {}).get(task_type)
    
    def render_prompt(self, task_type: TaskType, language: Language = None, **kwargs) -> str:
        """渲染指定任務和語言的完整 prompt"""
    ...
    def get_available_languages(self, task_type: TaskType) -> List[Language]:
        """獲取指定任務支援的語言列表"""
        available = []
