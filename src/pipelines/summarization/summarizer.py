"""ChapterSummarizer — LLM-based chapter and book summarization.

Uses tenacity retry (3 attempts, exponential backoff) following the
EntityExtractor pattern from Phase 2.
"""
...
class ChapterSummarizer:
    ...
    async def summarize_chapter(
        ...
        logger.info(
            "ChapterSummarizer: chapter=%d  summary_len=%d", chapter_number, len(summary)
        )
        return summary
    ...
    async def summarize_book(
        ...
        logger.info("ChapterSummarizer: book summary generated  len=%d", len(summary))
        return summary
