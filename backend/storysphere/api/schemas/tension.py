...
class TEUDetail(BaseModel):
    ...
    narrative_run_index: int | None = Field(
        default=None,
        description=(
            "1-based ordinal of the narrative-layer run this TEU sits in, "
            "within its chapter. A run breaks where narration switches between "
            "present and flashback, so TEUs sharing a chapter and a run index "
            "are continuous narration rather than independent corroboration. "
            "**Not a scene count**: a scene change inside one layer is "
            "invisible to it, and a chapter with no flashback is a single run "
            "however many scenes it holds (B-068). Null when the TEU's source "
            "event is no longer in the graph."
        ),
    )
    scene_index: int | None = Field(
        default=None,
        description=(
            "1-based ordinal of the scene this TEU sits in, within its chapter "
            "(B-068). Scenes come from the typographic dividers the prose is "
            "typeset with, so TEUs sharing a chapter and a scene index are one "
            "scene rather than independent corroboration. "
            "**Null does not mean 'one scene'** — it means the chapter carries "
            "no divider, so nothing is known about its scenes. A book typeset "
            "without dividers returns null for every TEU. Rendering null as a "
            "count of 1 asserts the opposite of what is known; show it as "
            "unavailable instead."
        ),
    )
